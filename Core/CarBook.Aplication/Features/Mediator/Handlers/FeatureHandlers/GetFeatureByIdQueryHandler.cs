...
{
	public class GetFeatureByIdQueryHandler(IRepository<Feature> repository) : IRequestHandler<GetFeatureByIdQuery, ServiceResult<GetFeatureByIdQueryResult>>
	{
		public async Task<ServiceResult<GetFeatureByIdQueryResult>> Handle(GetFeatureByIdQuery request, CancellationToken cancellationToken)
		{
			...
			if (feature == null)
			{
				return ServiceResult<GetFeatureByIdQueryResult>.Fail("Feature bulunamadı",HttpStatusCode.NotFound);
			}
